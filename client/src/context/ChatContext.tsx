...
import { ChatMessage, fetchChatMessages } from '../pages/Profile/types/api';
...
export const ChatProvider = ({ children, currentUserId }: { children: ReactNode; currentUserId: string }) => {
  ...
  const loadMessages = useCallback(
    (otherId: number) => {
      fetchChatMessages(Number(currentUserId), otherId)
  ...
  const sendMessage = useCallback(
    async (text: string) => {
      ...
      if (!sent) {
        dispatch({ type: 'addMessage', userId: Number(state.selected.id), payload: message });
        try {
          await sendChat(socket, Number(currentUserId), Number(state.selected.id), text);
        } catch {
          // ignore
        }
      }
  ...
  const value = { state, selectUser, sendMessage };
