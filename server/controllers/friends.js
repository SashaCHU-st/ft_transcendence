  ...
  try {
    ...
    if (hasUser2) {
      const friendAlready1 = db
        .prepare(
          `SELECT * FROM friends WHERE (user_id = ? AND friends_id = ?) OR (user_id = ? AND friends_id = ?)`
        )
...
export async function requestFriend(req, reply) {
  ...
  try {
    const checkRequest = db
      .prepare('SELECT * FROM friends WHERE friends_id = ? AND confirmReq = 0')
      .all(user_id);
    ...
    if (checkRequest) {
      ...
      return reply
        .code(200)
        .send({ message: 'There is request', checkRequest });
    } else {
